                                ...
                                <div class="row">
                                    <div class="col-sm-12 col-md-6">
                                        <div class="dt-button btn-group flex-wrap" >
                                                <a href="{{ route('nhapkho.them') }}" class="btn btn-success">Thêm mới</a>
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-6">
                                        ...
                                        <tr>
                                            ...
                                            @if(auth()->user()->isAdmin == 1)
                                                <td>
                                                    <div class="col-md-12">
                                                        @if($value->TrangThai->ten == 'Chưa duyệt')
                                                            <a href="{{ route('nhapkho.duyet', ['id' => $value->id]) }}"><i class="fas fa-check-circle"></i></a>
                                                        @else
                                                            <a href="{{ route('nhapkho.boduyet', ['id' => $value->id]) }}"><i class="fas fa-times-circle"></i></a>
                                                        @endif
                                                    </div>
                                                </td>
                                            @endif
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="invoice p-3 mb-3">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <h4 class="text-center">ĐƠN HÀNG NHẬP KHO</h4>
                            </div>
                        </div>
                        <!-- info row -->
                        <div class="row invoice-info">
                            ...
                            <div class="col-sm-6 invoice-col">
                                ...
                                <b>Nhà cung cấp: </b> <span id="nhacungcap"></span><br>
                                <b>Người nhập: </b> <span id="nguoinhap"></span><br>
                ...
                <div class="modal-footer justify-content-between">
                    ...
                    <div class="dt-button btn-group flex-wrap">
                        <a href="" onclick="print_current_page()" class="btn btn-success"><i class="fas fa-print"></i> In đơn</a>
                    </div>
                </div>
            </div>
            <!-- /.modal-content -->
...
@endsection
@section('jv_chitietdhnhap')
    <script>
        function openModal(id){
            $.ajax({
                url: "{{ route('nhapkho.xemchitiet', ['id' => ':id']) }}".replace(':id', id), // Đường dẫn đến route xử lý việc lấy dữ liệu chi tiết đơn hàng
                type: 'GET',
                success: function(response) {
                    // Hiển thị dữ liệu lấy được lên modal
                    $('#nhacungcap').text(response.tenncc);
                    $('#madon').text(response.id);
                    $('#nguoinhap').text(response.nguoinhap);
        ...
        /*function printOrder(){
            var orderId = $('#madon').text();
            console.log(orderId);
            var url = "".replace(':id', orderId);
